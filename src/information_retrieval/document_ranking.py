...
class TopicDocumentMatcher:
    ...
    def match_documents_to_query(self, query_topic_distribution):
        ...
        self.query_top_topics = self._extract_topics()

        # Filter documents that belong to the top topics
        relevant_documents = self._get_documents_in_top_topics(self.query_top_topics)

        # Rank documents within the relevant set
        ranked_documents = self._rank_documents_by_topic_similarity(
            query_topic_distribution, relevant_documents)

        return ranked_documents

    def _extract_topics(self, prob=0.5):
    ...
    def _get_documents_in_top_topics(self, top_topics):
    ...
    def _rank_documents_by_topic_similarity(self, query_topic_distribution, relevant_documents):
